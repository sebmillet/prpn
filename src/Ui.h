...
typedef enum {BEVAL_NULL,
  ...
  BEVAL_NEVER,      // While editing a string
...
//  DisplayStackLayout(const DisplayStackLayout&);
  ...
  void redefine_geometry(int, int, bool);
...
public:
  ...
  virtual void notify_ui_change() = 0;
  ...
  virtual int get_nb_menu_buttons() = 0;
  virtual void set_menu_button(const int&, const menu_button_t&) = 0;
