...
static vector<string> menu_buttons_display;
static int nb_menu_buttons_display;
static vector<menu_button_t> menu_buttons;
static int menu_button_page = 0;

typedef enum {MBT_NULL, MBT_MENUS, MBT_VARS} mb_type_t;
static mb_type_t menu_button_type = MBT_NULL;

struct group_menu_button_sets {
  string menu_name;
  const menu_button_t *mbs;
  int nb;
};

const menu_button_t mb_root[] = {
  {"MODE", "_MENU-MODE", "&&"},
  {"TRIG", "_MENU-TRIG", "&&"},
  {"LOGS", "_MENU-LOGS", "&&"},
  {"ARRAY", "_MENU-ARRAY", "&&"},
...
static const group_menu_button_sets all_buttons_menus[] = {
  {"ROOT", mb_root, sizeof(mb_root) / sizeof(*mb_root)},
  {"MODE", mb_mode, sizeof(mb_mode) / sizeof(*mb_mode)},
  {"TRIG", mb_trig, sizeof(mb_trig) / sizeof(*mb_trig)},
  {"LOGS", mb_logs, sizeof(mb_logs) / sizeof(*mb_logs)},
  {"ARRAY", mb_array, sizeof(mb_array) / sizeof(*mb_array)},
  {"BINARY", mb_binary, sizeof(mb_binary) / sizeof(*mb_binary)},
  {"COMPLX", mb_complx, sizeof(mb_complx) / sizeof(*mb_complx)},
  {"STRING", mb_string, sizeof(mb_string) / sizeof(*mb_string)},
  {"LIST", mb_list, sizeof(mb_list) / sizeof(*mb_list)},
  {"REAL", mb_real, sizeof(mb_real) / sizeof(*mb_real)},
  {"STACK", mb_stack, sizeof(mb_stack) / sizeof(*mb_stack)},
  {"CTRL", mb_ctrl, sizeof(mb_ctrl) / sizeof(*mb_ctrl)},
  {"BRANCH", mb_branch, sizeof(mb_branch) / sizeof(*mb_branch)},
  {"TEST", mb_test, sizeof(mb_test) / sizeof(*mb_test)}
};

void set_menu_buttons(const menu_button_t *mb, const int& nb) {
  menu_buttons.clear();
  for (int i = 0; i < nb; i++)
    menu_buttons.push_back(mb[i]);
  menu_button_page = 0;
}
...
static const BtnDescription btn_descriptions[] = {
  {1, "", 60, "_MENU1", "", 0, "_MENU1"},
  {1, "", 60, "_MENU2", "", 0, "_MENU2"},
  {1, "", 60, "_MENU3", "", 0, "_MENU3"},
  {1, "", 60, "_MENU4", "", 0, "_MENU4"},
  {1, "", 60, "_MENU5", "", 0, "_MENU5"},
  {1, "", 60, "_MENU6", "", 0, "_MENU6"},
  {-1, "", 0, "", "", 0, ""},
  {1, "USER", 80, "_MENU-USER", "", 0, "_NOOP"},
  {1, "HOME", 80, "HOME", "UP", 0, "UP"},
  {1, "MENU", 80, "_MENU-ROOT", "", 0, "_NOOP"},
  {1, "PREV", 80, "_MENU-PREV", "", 0, "_NOOP"},
  {1, "NEXT", 80, "_MENU-NEXT", "", 0, "_NOOP"},
  {-1, "", 0, "", "", 0, ""},
...
// Written into ~/.prpn/stackrc or alike depending on OS
  ...
  if (ofs.good()) {
    ...
    ofs << f << " STOF\n";
      // Uiset
    string uc = integer_to_string(ui_code);
    ofs << uc << " " CMD_PREFIX_NOSTD "UISET\n";
      // Stack items
...
void ui_init() {
  ...
  cmd_evals["_NOOP"] = BEVAL_BTN_NOOP;
    // Special case: ENTER
  ...
  // This function is called BEFORE the object ui_impl is created.
void ui_post_init_before_ui_build() {
  readrc();
}

  // And this function is called AFTER the object ui_impl is created.
void ui_post_init_after_ui_build() {
  ui_code_sequence++;
  ui_refresh_display();
}
...
static void refresh_path(const int& when) {
  ...
  if (when == REFRESH_PATH_POST && last_ui_code_sequence != ui_code_sequence) {
...
void refresh_menu_buttons() {
  ...
  nb_menu_buttons_display = ui_impl->get_nb_menu_buttons();
  int low = menu_button_page * nb_menu_buttons_display;
  int hi = low + nb_menu_buttons_display - 1;
  int limp1 = menu_buttons.size();
  menu_button_t m;
  menu_buttons_display.clear();
...
//    ui_impl->set_menu_button(i - low, m);
    menu_buttons_display.push_back(m.label);
  }
  ...
  if (!id || last_ui_code_sequence != ui_code_sequence) {
    debug_write_v("refresh_menu_buttons(): LABEL UPDATE");
    for (int i = low; i <= hi; i++) {
      if (i < limp1)
        m = menu_buttons[i];
      else {
...
void ui_notify_button_pressed(const char *c) {
  ...
  beval_t beval = BEVAL_DEFAULT;
  beval_t my_default = BEVAL_DEFAULT;
  string menu_cmd;

  if (string(c).substr(0, 5) == "_MENU") {
    debug_write_v("#001 menu command = %s", c);
    menu_cmd = string(c).substr(5);
    char c1 = (menu_cmd.length() >= 1 ? menu_cmd.at(0) : '\0');
    if (isdigit(c1)) {
      debug_write_v("#001-1: menu_cmd = %s", menu_cmd.c_str());
      int menu_button_number = string_to_integer(menu_cmd);
      menu_button_number = (menu_button_number < 1 ? 1 : menu_button_number);
      menu_button_number = (menu_button_number > nb_menu_buttons_display ? nb_menu_buttons_display : menu_button_number);
      int idx = menu_button_number + nb_menu_buttons_display * menu_button_page - 1;
      if (idx > static_cast<int>(menu_buttons.size() - 1))
        beval = BEVAL_BTN_NOOP;
      else {
        string *t;
        if (ui_dsl.get_status_shift())
          t = (menu_buttons[idx].alt_cmd == "&&" ? &menu_buttons[idx].main_cmd : &menu_buttons[idx].main_cmd);
        else
          t = &menu_buttons[idx].main_cmd;
        if (*t == "&&")
          t = &menu_buttons[idx].label;
        c = t->c_str();
        my_default = BEVAL_NOT_IN_EXPRESSION;
        debug_write_v("#001 Menu button command number #%i produce command '%s'", menu_button_number, c);
      }
    }
  }
  if (beval != BEVAL_BTN_NOOP) {
    string x = string(c);
    if (x.substr(0, 5) == "_MENU") {
      menu_cmd = x.substr(5);
      debug_write_v("#001 Now analyzing MENU command: %s", menu_cmd.c_str());
      if (menu_cmd.length() >= 2) {
        string m2 = menu_cmd.substr(1);
        for (unsigned int i = 0; i < sizeof(all_buttons_menus) / sizeof(*all_buttons_menus); i++) {
          if (m2 == all_buttons_menus[i].menu_name) {
            set_menu_buttons(all_buttons_menus[i].mbs, all_buttons_menus[i].nb);
            menu_cmd.clear();
            menu_button_type = MBT_MENUS;
            beval = BEVAL_BTN_NOOP;
            break;
          }
        }
      }
      if (menu_cmd == "-NEXT" || menu_cmd == "-PREV") {
        if (!menu_buttons.empty()) {
          int n = (menu_buttons.size() - 1) / nb_menu_buttons_display + 1;
          if (menu_cmd == "-NEXT")
            menu_button_page = (menu_button_page + 1) % n;
          else {
            menu_button_page--;
            if (menu_button_page < 0)
              menu_button_page = n - 1;
          }
        }
        beval = BEVAL_BTN_NOOP;
      } else if (menu_cmd == "-USER") {
        menu_button_type = MBT_VARS;
        menu_button_page = 0;
        beval = BEVAL_BTN_NOOP;
      }
    } else {
  ...
  if (beval == BEVAL_BTN_SHIFT)
    ...
    ui_impl->display_help(DH_MAIN);
          ...
          *c == '\0') &&
        beval != BEVAL_NEVER &&
        beval != BEVAL_NULL && beval != BEVAL_BTN_NOOP &&
        (beval != BEVAL_NOT_IN_EXPRESSION || typein_status != TYPEIN_EXPRESSION)) {
    ui_flush_input(ui_impl->get_string(), c);
  } else if (beval != BEVAL_NULL && beval != BEVAL_BTN_NOOP) {
    if (beval != BEVAL_NEVER && typein_status != TYPEIN_EXPRESSION && !ui_impl->space_at_the_left_of_the_cursor())
      ui_impl->insert_text(" ");
